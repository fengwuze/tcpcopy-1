...
tc_check_sess_timeout(tc_event_timer_t *ev)
{
    ...
    if (s != NULL) {
        ...
        tc_log_debug1(LOG_INFO, 0, "check timeout:%u", ntohs(s->src_port));
        now = tc_time();
        ...
        if (result != OBSOLETE) {
            if (s->sm.state & SND_REQ) {
                if ((now - s->req_snd_con_time) >= CHECK_SESS_TIMEOUT) {
...
tc_snd_pack(tc_sess_t *s, tc_iph_t *ip, tc_tcph_t *tcp, bool client)
{
    ...
    if (s->cur_pack.cont_len > 0) {
        ...
        if (!s->sm.pool_loop_used) {
            if (s->cur_pack.cont_len <= TC_UPOOL_MAXV) {
...
send_router_info(tc_sess_t *s, uint16_t type)
{
    ...
    if (fd > 0) {
        ...
        if (tc_socket_snd(fd, (char *) &msg, MSG_CLT_SIZE) != TC_ERR) {
            ...
            tc_log_info(LOG_ERR, 0, "msg send error");
            return false;
        }
    } else {
        tc_log_debug1(LOG_WARN, 0, "sock invalid:%u", ntohs(s->src_port));
        return false;
    }
}
#endif
...
retrans_pack(tc_sess_t *s, uint32_t expected_seq)
{
    ...
    while (ln) {
        ...
        if (cont_len > 0) {
            if (cur_seq == expected_seq) {
                ...
                if (before(cur_seq, s->rep_ack_seq)) {
                    ...
                    tc_log_info(LOG_NOTICE, 0, "no retrans pack:%u", 
                            ntohs(s->src_port));
                    break;
                }
            }
        ...
        if (find_and_retransmit) {
            tc_log_info(LOG_INFO, 0, "retransmit, len:%u,p:%u", cont_len, 
                    ntohs(s->src_port));
...
static void 
send_faked_third_handshake(tc_sess_t *s, tc_iph_t *ip, tc_tcph_t *tcp)
{
    tc_iph_t       *f_ip;
    tc_tcph_t      *f_tcp;
    unsigned char  *p, frame[FFRAME_LEN];
...
static void
tc_delay_ack(tc_sess_t *s, tc_event_timer_t *ev)
{
    int   rtt;
    ...
    if (!s->sm.sess_over) {
        ...
        tc_log_info(LOG_WARN, 0, "over, ev timer:%u", ntohs(s->src_port));
    }
}
...
shrink_rtt(tc_sess_t *s) 
{
    if (!s->sm.internal_usage) {
        s->rtt = s->rtt >> 1;
        tc_log_debug2(LOG_INFO, 0, "srtt:%u:%u", s->rtt, ntohs(s->src_port));
        s->rtt = s->rtt ? s->rtt:1;
    }
}
...
tc_lantency_ctl(tc_event_timer_t *ev)
{
    ...
    if (s != NULL) {
        ...
        } else if (s->sm.timer_type == TYPE_TIMEOUT) {
            if (s->sm.candidate_rep_wait && !s->sm.rep_payload_type) {
                tc_log_debug1(LOG_INFO, 0, "activate:%u", ntohs(s->src_port));
                proc_clt_pack_from_buffer(s);
            }
...
proc_backend_packet(tc_sess_t *s, tc_iph_t *ip, tc_tcph_t *tcp)
{
    ...
    if (!tcp->rst) {
        ...
        if (s->cur_pack.cont_len > 0) {
            ...
            s->rep_rcv_con_time = tc_time();
            s->target_ack_seq = htonl(s->cur_pack.seq + s->cur_pack.cont_len);
        } else {
            s->target_ack_seq = tcp->seq;
        }
...
static void
proc_bak_syn(tc_sess_t *s, tc_iph_t *ip, tc_tcph_t *tcp)
{
    bool     pack_sent;
    uint16_t size_tcp;
    ...
    if (!s->sm.fake_syn) {
        ...
        send_faked_third_handshake(s, ip, tcp);
        proc_clt_pack_from_buffer(s);
        s->sm.state |= ESTABLISHED;
    }
}
...
proc_outgress(unsigned char *pack)
{
    ...
    if (s) {
        ...
        if (s->sm.state >= SYN_CONFIRM || tcp->syn) {
            ...
            tc_log_info(LOG_INFO, 0, "prev pack:%u", ntohs(s->src_port));
            if (tcp->rst) {
                sess_post_disp(s, true);
            } else {
                send_faked_rst(s, ip, tcp);
...
#endif
            tc_log_debug2(LOG_DEBUG, 0, "record rtt base:%llu,p:%u",
...
#endif
        tc_log_debug2(LOG_INFO, 0, "rtt:%llu,p:%u", s->rtt, ntohs(s->src_port));
    }
}
...
static int 
continue_diag(tc_sess_t *s, tc_iph_t *ip, tc_tcph_t *tcp)
{
...
static bool
is_wait_greet(tc_sess_t *s, tc_iph_t *ip, tc_tcph_t *tcp)
{
    if (s->sm.need_rep_greet) {
        return true;
    }
    ...
    if (s->sm.req_ack_snd) {
        ...
        if (after(s->cur_pack.ack_seq, s->req_ack_snd_seq) && 
                s->cur_pack.seq == s->target_nxt_seq) 
        {
...
check_wait_prev_packet(tc_sess_t *s, tc_iph_t *ip, tc_tcph_t *tcp)
{
    ...
    if (s->cur_pack.seq == s->target_nxt_seq) {
        return PACK_CONTINUE;
    } else if (after(s->cur_pack.seq, s->target_nxt_seq)) {
        tc_log_debug3(LOG_DEBUG, 0, "lost pack, seq:%u, expect:%u,p:%u", 
                s->cur_pack.seq, s->target_nxt_seq, ntohs(s->src_port));
        return PACK_STOP;
    } else {
        retransmit_seq = s->target_nxt_seq - s->cur_pack.cont_len;
        if (!after(s->cur_pack.seq, retransmit_seq)) {
            tc_log_debug2(LOG_DEBUG, 0, "exp seq:%d, clt retransmit:%u",
                    s->target_nxt_seq, ntohs(s->src_port));
            if (tcp->fin) {
                s->sm.delay_snd = 1;
            }
    ...
    if (!s->sm.candidate_rep_wait) {
        if (s->cur_pack.cont_len > 0) {
            ...
            if (clt_settings.default_rtt == 0) {
                ...
                tc_log_debug2(LOG_DEBUG, 0, "rtt:%llu,p:%u", s->rtt, 
                        ntohs(s->src_port));
            }
            if (s->rtt <= 1) {
...
proc_clt_packet(tc_sess_t *s, tc_iph_t *ip, tc_tcph_t *tcp)
{
    ...
    if (s->sm.state < SND_REQ && is_wait_greet(s, ip, tcp)) {
        return PACK_STOP;
    }
...
check_ingress_pack_needed(tc_iph_t *ip)
{
    ...
    size_tcp = tcp->doff << 2;
    if (size_tcp < TCPH_MIN_LEN) {
        tc_log_info(LOG_WARN, 0, "Invalid TCP header: %d bytes,pack len:%d",
                size_tcp, tot_len);
        return is_needed;
    }
                ...
                tcp->dest, CHECK_DEST)) 
    {
        ...
        if (tot_len >= hlen) {
            ...
            tc_log_info(LOG_WARN, 0, "bad tot:%d, hlen:%d", tot_len, hlen);
        }
    } 
...
output_stat()
{
    ...
    if (tc_stat.start_p_time != 0) {
        ...
        tc_log_info(LOG_NOTICE, 0, "send fin:%llu,send reset:%llu",
                tc_stat.fin_sent_cnt, tc_stat.rst_sent_cnt);
        ...
        tc_log_info(LOG_NOTICE, 0, "successful retransmit:%llu", 
                tc_stat.retrans_succ_cnt);
...
proc_clt_pack_from_buffer(tc_sess_t *s)
{
    ...
    tc_log_debug3(LOG_INFO, 0, "slide_win_packs size:%u, rtt:%u,p:%u", 
            s->slide_win_packs->size, s->rtt, ntohs(s->src_port));
...
proc_ingress(tc_iph_t *ip, tc_tcph_t *tcp)
{
    ...
    if (!tcp->syn) {
        ...
        candi_prev_pack_detected = false;
        ...
        if (s) {
            ...
            if (s->sm.timeout) {
                if (after(ntohl(tcp->seq), s->req_con_snd_seq)) {
                    candi_prev_pack_detected = true;
                }
            }
        }

        if (s && !candi_prev_pack_detected) {
