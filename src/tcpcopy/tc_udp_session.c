...
ip_fragmentation(tc_iph_t *ip, tc_udpt_t *udp)
{
    ...
    ret = tc_raw_socket_send(tc_raw_socket_out, tmp_ip, 
...
bool tc_proc_ingress(tc_iph_t *ip, tc_udpt_t *udp)
{
    ...
    if (tot_len > clt_settings.mtu) {
        ...
        ip_fragmentation(ip, udp);
    } else {

        ret = tc_raw_socket_send(tc_raw_socket_out, ip, 
                tot_len, ip->daddr);
        if (ret == TC_ERR) {
